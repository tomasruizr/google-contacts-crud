...
var CreateContact = function(contactInfo) {
    ...
    this._groups = contactInfo.groups || [];
...
CreateContact.prototype.getXml = function() {
    ...
    // ADDED BY TOMAS RUIZ TO SUPPORT THE ASSOCIATIONS WITH GROUPS
    if (this._groups.length) {
        obj["gContact:groupMembershipInfo"] = [];
        for (var gcont = 0; gcont < this._groups.length; gcont++) {
            obj["gContact:groupMembershipInfo"].push({
                '@deleted': 'false',
                '#text': this._groups[gcont]
            })
        }
    }
